#!/usr/bin/env python3
"""
全ノーベル化学賞論文を Sci-Hub から網羅的にダウンロード
1901年～2025年（125年間）のすべての論文を収集
...
# CanvasのJSONファイル名に合わせます
DATA_FILE = "nobel_chemistry_laureates.json"
STATS_FILE = "download_statistics.json"

# Sci-Hub ミラーリスト (複数のバックアップあり)
SCI_HUB_MIRRORS = [
    "https://sci-hub.se/",
    "https://sci-hub.st/",
    "https://sci-hub.ru/",
...
# ロギング設定
# ==========================================
# Windowsの日本語環境(CP932)でのクラッシュを防ぐため、
# 1. コンソール出力用のハンドラではUnicode特殊記号（絵文字や丸囲み文字）の使用を避けます。
# 2. ファイル出力用のハンドラには明示的に encoding='utf-8' を指定します。
...
def download_all_papers():
    ...
    if not os.path.exists(DATA_FILE):
        ...
        logger.error("Canvas から JSON データをコピーし、同じフォルダに保存してください。")
        return None
    ...
    logger.info("=" * 70)
    logger.info("ノーベル化学賞 論文一括ダウンロード開始")
    logger.info("期間: 1901年 ～ 2025年 (125年)")
    logger.info(f"対象: {len(nobel_data)}件")
    logger.info("=" * 70)
    ...
    for idx, record in enumerate(nobel_data, 1):
        ...
        names = record.get('names', 'Unknown')
        doi = record.get('doi', '').strip()
        
        # 進捗表示
        progress = f"[{idx:3d}/{len(nobel_data)}]"
        
        # DOI がない、または "No award" の場合
        if not doi or doi == 'No award':
            logger.info(f"{progress} [SKIP] スキップ ({year}年): DOIなし / 受賞なし")
            skip_count += 1
            stats["no_doi"] += 1
            continue
        
        logger.info(f"{progress} [{year}年] {title[:60]}")
        
        # ファイル名を生成
        safe_title = sanitize_filename(title)[:40]
        filename = f"{SAVE_DIR}/[{year:04d}]_{safe_title}.pdf"
